...
{
	Nimbus.clonedNodesContainer.appendChild(elem.cloneNode(true));
}
...
function logMutations(mutations)
{
	...
	for(let i = 0, ii = mutations.length; i < ii; i++)
	{
		...
		else if(mutation.type === "attributes")
		{
			...
			{
				console.log(`%c${mutation.attributeName}: %c${createSelector(mutation.target)}`, "color: #0C0", "color: #CCC; background: #444");
				cloneElement(mutation.target);
			}
		}
	}
}
